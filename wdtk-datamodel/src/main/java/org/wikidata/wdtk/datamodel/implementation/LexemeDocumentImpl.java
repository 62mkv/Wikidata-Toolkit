...
public class LexemeDocumentImpl extends StatementDocumentImpl implements LexemeDocument {
     ...
     * Constructor.
     *
     * @param id              the id of the le that data is about
     * @param lexicalCategory the lexical category of the lexeme
     * @param language        the language of the lexeme
     * @param lemmas          the list of lemmas of this lexeme, with at most one
     *                        lemma for each language code
     * @param statements      the list of statement groups of this lexeme; all of them must
     *                        have the given id as their subject
     * @param forms           the list of the forms of this lexeme.
     * @param senses          the list of the senses of this lexeme.
     * @param revisionId      the revision ID or 0 if not known; see
    ...
    @Override
    public FormDocument createForm(List<MonolingualTextValue> representations, List<ItemIdValue> grammaticalFeatures, List<StatementGroup> statements) {
        FormIdValue newFormId = FormIdValue.NULL;
/*
            newFormId = new FormIdValueImpl(entityId + "-F" + nextFormId, siteIri);
            nextFormId++;
 */

        return new FormDocumentImpl(newFormId, representations, grammaticalFeatures,
                statements, revisionId);
    }

    @Override
    public FormDocument createForm(List<MonolingualTextValue> representations, List<ItemIdValue> grammaticalFeatures) {
        return this.createForm(representations, grammaticalFeatures, Collections.emptyList());
    }

    @Override
    public FormDocument createForm(List<MonolingualTextValue> representations) {
        return this.createForm(representations, Collections.emptyList());
    }

    @Override
    public LexemeDocument withForm(FormDocument form) {
        if (!formBelongsToThisLexeme(form)) {
    ...
    private boolean formBelongsToThisLexeme(FormDocument form) {
        if (FormIdValue.NULL.equals(form.getEntityId())) {
            return true;
        }
        return form.getEntityId().getLexemeId().equals(getEntityId());
    }
